...
def part_configuration_selector(unit_type):
    ...
    match unit_type:
        case "AA2JE":
            ...
            with open('Part Cheking Configuration AA2JE.csv', 'r') as file:
        ...
        case "AA7NE":
            ...
            df = pd.read_csv('masking_coordinate_AA7NE.csv')
            with open('Part Cheking Configuration AA7NE.csv', 'r') as file:
                reader = csv.reader(file, skipinitialspace=True, quoting=csv.QUOTE_NONE)
                for row in reader:
                    row_count = row_count + 1
                    part_configuration_list_buffer.append(row)
                return part_configuration_list_buffer
        case "ADXDE":
           # client.write_register(address=4000,value=30,slave=1)
            df = pd.read_csv('masking_coordinate_ADXDE.csv')
...
def save_record_csv(barcode_number_590, barcode_number_815, NG_OK, Remark, PartDirectory, Station):
    global annotated_frame
    Now = datetime.datetime.now()
    cursor = cnxn.cursor()
    cursor.execute("INSERT INTO [" + table + "](DateTime,UnitNumber,Remark,Status,Directory,Station) values ('" + str(
        Now) + "','" + str(barcode_number_590) + "','" + str(Remark) + "','" + str(NG_OK) + "','" + str(
        PartDirectory) + "','" + str(Station) + "')")
...
def read_table():
    cursor = cnxn.cursor()
    cursor.execute(
        "SELECT F590SerielNumber,F815SerielNumber FROM F810SerielNumber where Id=(Select MAX(Id) from F810SerielNumber) order by id desc")
    for row in cursor.fetchall():
        return (row)


def save_in_server(barcode_number_590, barcode_number_815, NG_OK, part_ng_count, ScratchesMoistureTotal, path,
                   station, new_now, image_list_part):
    global annotated_frame
    try:
        if checkbox_status == 0:
            save_record_csv(barcode_number_590=barcode_number_590,
                            barcode_number_815=barcode_number_815, NG_OK=NG_OK,
                            ...
                            Station=station) ##nanti tukar jadi variable!!!!!!!!!!!!!!!
        else:
            save_record_csv(barcode_number_590=barcode_number_590,
                            barcode_number_815="", NG_OK=NG_OK,
                            Remark="Found:" + str(part_ng_count) + " PartNG || " + str(
                                sum(ScratchesMoistureTotal)) + " AppNG",
                            PartDirectory=save_record_directory(path,
                                                                new_now=new_now) + new_now.strftime(
...
def task1():  # get_frame
    ...
    while cap.isOpened():
        ...
        if imgmasking=='1':
            frame = masking_program.masking_frame(area_variable=area_variable, frame_resize=frame_resize, df=df,
                                                  show_masking_number=False)
        else:
            frame = frame_resize
...
def task2():
    global area, annotated_frame, cls_list, PartCheckingFinished, frame_copy, CheckBox_ID, checkbox_status, button_status, barcode_number_590, barcode_number_815, NG_OK, part_ng_count, ScratchesMoistureTotal, path, note, new_now, image_list_part,area_variable,ng_string_total,dataset_colection
    ...
    checkbox_status = 1
    button_status = ""
    client.write_coil(address=345, value=0, slave=1)  # i/o checking
    while (1):
        while (button_status == ""):
            print("PLEASE SELECT PART CONFIGURATION!!!")
            time.sleep(0.5)
        while (step < len(PartChecking1)):
            rr = client.read_coils(address=320, count=1, slave=1)
            if (rr.bits[0] == 1):
                if flag == 0:
                ...
                if process_frame(PartChecking1[step], PartChecking2[step], PartChecking3[step],
                    ...
                    annotated_frame = cv2.putText(annotated_frame, 'OK', (0, 50), cv2.FONT_HERSHEY_SIMPLEX, 2,
                                                  (0, 0, 0), 3, cv2.LINE_AA)
                    annotated_frame = cv2.putText(annotated_frame, str(step + 1), (100, 50), cv2.FONT_HERSHEY_SIMPLEX, 2,
                ...
                if (rr.bits[0] == 1):
                    ...
                    if "\n" in TextBox_ID.get("1.0", "end-1c"):
                        ...
                        if NG_OK == True:
                            path = OK_PATH
                            NG_OK = "OK"
                            client.write_coil(address=340, value=1, slave=1)  # green lamp

                            annotated_frame = cv2.imread('OK_DISPLAY.JPG')
                            save_in_server(barcode_number_590=barcode_number_590, barcode_number_815="",
                                           part_ng_count=part_ng_count,
                                           ScratchesMoistureTotal=ScratchesMoistureTotal,
                                           path=path,station=station, new_now=new_now, NG_OK=NG_OK,
                                           image_list_part=image_list_part)

                        else:
                            client.write_coil(address=345, value=1, slave=1)  # i/o checking
                            path = NG_PATH
                            NG_OK = "NG"
                            client.write_coil(address=342, value=1, slave=1)  # red lamp
                            annotated_frame = cv2.imread('NG_DISPLAY.JPG')

                            client.write_coil(address=351, value=0, slave=1)  # CLEAR BUTTON
                            client.write_coil(address=352, value=0, slave=1)  # CLEAR BUTTON

                            if (len(image_list_temp_show) != 0):
                                while (show_ng == 0):
                                    rx = client.read_coils(address=350, count=1, slave=1)
                                    if (rx.bits[0] == 1):
                                        show_ng = 1
                                        time.sleep(0.1)
                                        ...
                                        if (len(image_list_temp_show) < 10):
                                            ...
                                            while (show_ng_judgement == 0):
                                                rx2 = client.read_coils(address=351, count=1, slave=1)
                                                rx3 = client.read_coils(address=352, count=1, slave=1)
                                                if (rx2.bits[0] == 1):
                                                    time.sleep(0.1)
                                                    client.write_coil(address=345, value=0, slave=1)  # i/o checking
                                                    show_ng_judgement = 1
                                                    ...
                                                    save_in_server(barcode_number_590=barcode_number_590,
                                                                   ...
                                                                   part_ng_count=part_ng_count,
                                                                   ScratchesMoistureTotal=ScratchesMoistureTotal,
                                                                   path=path, station=station, new_now=new_now,
                                                                   NG_OK=NG_OK,
                                                                   image_list_part=image_list_part)
...
def task3():
    ...
    Check2 = Checkbutton(win, text="DATASET COLLECTION?", variable=var3, onvalue=1, offvalue=0, command=dataset_collection,
                         justify=LEFT, bd=5, font='Helvetica 12 bold')
    ...
    radio_button2 = Radiobutton(win, text="AA7NE", variable=var2, value="AA7NE", indicator=2, background="light blue",
                                command=display_input, font='Helvetica 12 bold')
...
if __name__ == "__main__":
    try:
        ...
        print("Model: " + model_inference + " Model2: " + model2_inference)
...
"""
                        results = zxingcpp.read_barcodes(for_barcode)
                        for result in results:
                            finalresult = result.text
                        if len(results) == 0:
                            finalresult = "Could not find any barcode."
